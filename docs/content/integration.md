...
FastAPI's own `/docs` and `/openapi.json` also collide with `/{action}`; `create_app()` disables them. In your application either keep them and mount the connector under a prefix, or disable them: `FastAPI(docs_url=None, redoc_url=None, openapi_url=None)`. The connector's documented API is the [OpenAPI document](api-swagger/index.md) generated from its schemas.
